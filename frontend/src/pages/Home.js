...
import './Home.css';
import { FaWallet, FaMoneyBillWave, FaHistory, FaDownload } from 'react-icons/fa';
...
const Home = ({ user, setUser }) => {
  ...
  const fetchCartela = async (luckyNumber) => {
    try {
      const response = await axios.get(`${API_URL}/api/cartela/${luckyNumber}`);
  ...
  const fetchTransactions = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/user/transactions/${user.id}`, {
  ...
  const fetchGameHistory = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/user/game-history/${user.id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleDeposit = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/user/deposit-request`, {
        userId: user.id,
      ...
      if (response.data.success) {
        ...
        setDepositSms('');
        const balanceResponse = await axios.get(`${API_URL}/api/user/balance/${user.id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/user/withdraw`, {
        userId: user.id,
  ...
  const handleNumberClick = async (number) => {
    ...
    if (selectedNumbers.includes(number)) {
      ...
      if (balance < requiredBalance) {
        toast.error(`Insufficient balance! Need ${requiredBalance} Birr for ${selectedNumbers.length + 1} cartela(s).`);
        return;
      }
  ...
  useEffect(() => {
    const fetchAdvertisement = async () => {
      try {
        const response = await axios.get(`${API_URL}/api/admin/advertisement`);
    ...
    newSocket.on('game-update', (data) => {
      ...
      setWinnerAmount(data.winnerAmount || 0);
      console.log('Game update - Winner amount:', data.winnerAmount);
    });
    ...
    return (
      <div key={index} className="selected-cartela">
        <div className="cartela-header">
          ...
          <button className="remove-cartela-btn" onClick={() => handleNumberClick(cartela.lucky_number)}>
            ✕ Remove
          </button>
        </div>
    ...
    <div className="home-container">
      ...
      <div className="header">
        ...
        <div className="game-info">
          <div className="winner-prize">🏆 Winner Gets: {Math.max(0, winnerAmount).toFixed(2)} Birr</div>
          <div className="timer">⏰ Next Game: {timeRemaining}s</div>
        </div>
      </div>
      ...
      <div className="purple-footer">
        <button className="footer-icon-btn" onClick={() => { fetchTransactions(); setShowBalanceModal(true); }}>
          <FaWallet className="footer-icon" />
          <span className="footer-label">Balance</span>
        </button>
        <button className="footer-icon-btn" onClick={() => { setWithdrawAmount(''); setWithdrawPhone(''); setShowWithdrawModal(true); }}>
          <FaDownload className="footer-icon" />
          <span className="footer-label">Withdraw</span>
        </button>
        <button className="footer-icon-btn" onClick={() => { setDepositAmount(null); setDepositSms(''); setShowDepositModal(true); }}>
          <FaMoneyBillWave className="footer-icon" />
          <span className="footer-label">Deposit</span>
        </button>
        <button className="footer-icon-btn" onClick={() => { fetchGameHistory(); setShowHistoryModal(true); }}>
          <FaHistory className="footer-icon" />
          <span className="footer-label">History</span>
        </button>
      </div>
      
      {/* Modals - Balance, Withdraw, Deposit, History */}
      {showBalanceModal && (
        <div className="modal-overlay" onClick={() => setShowBalanceModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h2>💰 Balance & Transactions</h2></div>
            <div className="modal-body">
              <div className="current-balance"><h3>Current Balance</h3><p className="balance-amount">{balance} Birr</p></div>
              <div className="transactions-list"><h3>Transaction History</h3>
      ...
      {showWithdrawModal && (
        <div className="modal-overlay" onClick={() => { setShowWithdrawModal(false); setWithdrawAmount(''); setWithdrawPhone(''); }}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h2>💸 Withdraw Request</h2></div>
            <div className="modal-body">
              <div className="current-balance"><p>Available Balance: <strong>{balance} Birr</strong></p><p className="min-withdraw">Minimum withdrawal: 100 Birr</p></div>
              <div className="input-group"><label>Amount (Birr)</label><input type="number" placeholder="Enter amount" value={withdrawAmount} onChange={(e) => setWithdrawAmount(e.target.value)} min="100" max={balance} /></div>
              <div className="input-group"><label>Phone Number (Telebirr)</label><input type="tel" placeholder="2519xxxxxxxx" value={withdrawPhone} onChange={(e) => setWithdrawPhone(e.target.value)} /></div>
              <button className="submit-btn" onClick={handleWithdraw}>Request Withdrawal</button>
            </div>
            <button className="modal-close-btn" onClick={() => { setShowWithdrawModal(false); setWithdrawAmount(''); setWithdrawPhone(''); }}>Close</button>
          </div>
        </div>
      )}
      
      {showDepositModal && (
        <div className="modal-overlay" onClick={() => { setShowDepositModal(false); setDepositAmount(null); setDepositSms(''); }}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h2>📱 Deposit Money</h2></div>
            <div className="modal-body">
              <div className="deposit-amounts"><p>Select Amount:</p><div className="amount-buttons">
              ...
              <div className="telebirr-info-modal"><p>📱 Send money to Telebirr: <strong>09XX-XXX-XXX</strong></p><p>After sending, copy the SMS confirmation and paste below:</p></div>
              <div className="input-group"><label>Paste Telebirr SMS</label><textarea rows="4" placeholder="Paste the SMS you received from Telebirr here..." value={depositSms} onChange={(e) => setDepositSms(e.target.value)} /></div>
              <button className="submit-btn" onClick={handleDeposit} disabled={depositStatus === 'processing'}>{depositStatus === 'processing' ? 'Submitting...' : 'Submit Deposit Request'}</button>
            </div>
            <button className="modal-close-btn" onClick={() => { setShowDepositModal(false); setDepositAmount(null); setDepositSms(''); }}>Close</button>
          </div>
        </div>
      )}
      ...
      {showHistoryModal && (
        <div className="modal-overlay" onClick={() => setShowHistoryModal(false)}>
          <div className="modal-content large" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h2>📜 Game History</h2></div>
            <div className="modal-body">
              {gameHistory.length === 0 ? <p className="no-data">No games played yet</p> : (
                <div className="history-table">
                  <table><thead><tr><th>Date</th><th>Game #</th><th>Cartelas</th><th>Status</th><th>Prize</th></tr></thead>
                  <tbody>
                    {gameHistory.map((game, idx) => (
                      <tr key={idx}>
                        ...
                        <td className={game.is_winner ? 'winner-status' : 'loss-status'}>{game.is_winner ? '🏆 WON' : '❌ Lost'}</td>
                        <td className={game.is_winner ? 'winner-amount' : ''}>{game.is_winner ? `${game.prize_amount} Birr` : '-'}</td>
                      </tr>
                    ))}
                  </tbody></table>
                </div>
